...
import { Hit as MeilisearchHit } from "meilisearch";

function Hit({ hit }: { hit: MeilisearchHit<Movie> }) {
  return (
    <div className="md:col-span-1 border border-gray-400 rounded-md h-full p-4 flex flex-col">
      <div className="flex flex-col items-center">
        ...
        <img src={hit.poster} alt={hit.title} className="w-32 h-auto my-6" />
      </div>
...
export default function HomeClient() {
  return (
    <InstantSearch>
      ...
      <Hits<Movie> hitComponent={({ hit }) => <Hit key={hit.id} hit={hit} />} />
    </InstantSearch>
  );
}
